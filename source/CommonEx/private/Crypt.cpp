...
using namespace CommonEx::Crypt;

/*------------------------------------------------------------
		Crypt (needs implentation update)
 ------------------------------------------------------------*/
#define SHA_DIGEST_LENGTH 20

UINT32 Crypt::Sha_l::CircularShift(int bits, UINT32 word)
{
	return ((((word) << bits) & 0xFFFFFFFF) | ((word) >> (32 - bits)));
}
int Crypt::Sha_l::Result()
{
	if (Corrupted != 0)
		return 0;
...
void Crypt::Sha_l::Input(UINT8* messageArray, int Length)
{
	if (Length == 0)
		return;
	...
	{
		MessageBlock[(size_t)MessageBlockIndex++] = messageArray[counter];
		LengthLow += 8;
...
void Crypt::Sha_l::ProcessMessageBlock()
{
...
Crypt::Sha_l::Sha_l(UINT8* src, int size)
{
	LengthLow = 0;
	LengthHigh = 0;
	MessageBlockIndex = 0;
...
void Crypt::XorKey(UINT8* result, UINT8* key1, int Length1, UINT8* key2, int Length2)
{
	int Length = Length1 < Length2 ? Length1 : Length2;
	for (int i = 0; i < Length; i++)
		result[i] = (UINT8)(key1[i] ^ key2[i]);
}
void Crypt::ShiftKey(UINT8* result, UINT8* src, int n, bool direction)
{
	int Length = 128;
	for (int i = 0; i < Length; i++)
		if (direction)
...
void Crypt::CryptorKey::set(int size, int maxPos)
{
	Pos1 = Key = Sum = 0;
	Size = size;
	Pos2 = MaxPos = maxPos;
...
void Crypt::Cryptor::FillKey(UINT8* src, UINT8* buf)
{
	for (int i = 0; i < 680; i++)
		buf[i] = src[i % 128];
	buf[0] = 128;
}
void Crypt::Cryptor::GenerateKey(UINT8* src)
{
	UINT8 buf[680];
	FillKey(src, buf);
...
void Crypt::Cryptor::ApplyCryptor(UINT8* buffer, int size)
{
...
//bool Crypt::Crypt::Session::send_server_key(Connection * c, UINT8 key_no)
//{
...
bool Crypt::Session::fill_key(UINT8 key[128], UINT8 key_id)
{
	if (key_id == 1)
	{
