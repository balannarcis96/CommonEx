...
SGlobalMath::SGlobalMath()
{
	...
	//Canche unit angles
	float* Cursor = CachedUnitAngles;
	INT j = 0;
	SAngle SourceAngle = 0;
	do {
		float CachedAngle = ((SourceAngle.angle - 32768) * 0.000030517578f) * PI;

		float SinValue = appSin(CachedAngle);
		float CosValue = appCos(CachedAngle);
...
//Max angle(360).	
const SAngle SAngle::MaxAngle = UINT16_MAX;
