...
namespace CommonEx
{
	//String function to convert between UTF16 <-> UTF8
	...
	struct ScopeGuard
	{
		ScopeGuard(const TLambda& OnExit) : OnExit(OnExit) {}
		ScopeGuard(ScopeGuard&& Other) : OnExit(std::move(Other.OnExit)) {}
		~ScopeGuard()
		{
			if (IsValid())
			{
	...
	template<typename TLambda>
	constexpr ScopeGuard<TLambda> MakeScopeGuard(const TLambda& Lambda)noexcept
	{
		return ScopeGuard<TLambda>(Lambda);
	}

#define S_GUARD(lambda) auto SGuard = MakeScopeGuard(lambda)
}
