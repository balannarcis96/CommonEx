...
*/
inline int32_t appTrunc(float F)
{
...
*/
inline int32_t appRound(float F)
{
...
*/
inline int32_t appFloor(float F)
{
	// Note: the x2 is to workaround the rounding-to-nearest-even-number issue when the fraction is .5
	return _mm_cvt_ss2si(_mm_set_ss(F + F - 0.5f)) >> 1;
}
...
*/
inline int32_t appCeil(float F)
{
	// Note: the x2 is to workaround the rounding-to-nearest-even-number issue when the fraction is .5
	return -(_mm_cvt_ss2si(_mm_set_ss(-0.5f - (F + F))) >> 1);
}

inline float appCopySign(float A, float B) { return std::copysign(A, B); }
...
inline BOOL appIsNaN(float A) { return _isnan(A) != 0; }
inline BOOL appIsFinite(float A) { return _finite(A); }
inline int32_t appRand() { return rand(); }
inline void appRandInit(int32_t Seed) { srand(Seed); }
inline float appFrand() { return rand() / (float)RAND_MAX; }
inline void appSRandInit(int32_t Seed) { GSRandSeed = Seed; }
...
inline float appSRand()
{
	...
	float Result;
	*(int32_t*)&Result = (*(int32_t*)&SRandTemp & 0xff800000) | (GSRandSeed & 0x007fffff);
...
inline ulong_t appFloorLog2(ulong_t Value)
{
...
*/
inline ulong_t appCountLeadingZeros(ulong_t Value)
{
	if (Value == 0) return 32;
	return 31 - appFloorLog2(Value);
}

inline float appFloatSelect(float Comparand, float ValueGEZero, float ValueLTZero)
{
	return Comparand >= 0.f ? ValueGEZero : ValueLTZero;
}

template< class T, class U > inline T Lerp(const T& A, const T& B, const U& Alpha)
{
	return (T)(A + Alpha * (B - A));
}

template< class T, class U > inline T BiLerp(const T& P00, const T& P10, const T& P01, const T& P11, const U& FracX, const U& FracY)
{
	return Lerp(
		Lerp(P00, P10, FracX),
		Lerp(P01, P11, FracX),
...
template< class T, class U > inline T CubicInterp(const T& P0, const T& T0, const T& P1, const T& T1, const U& A)
{
	const float A2 = A * A;
	const float A3 = A2 * A;
...
template< class T, class U > inline T CubicInterpDerivative(const T& P0, const T& T0, const T& P1, const T& T1, const U& A)
{
...
template< class T, class U > inline T CubicInterpSecondDerivative(const T& P0, const T& T0, const T& P1, const T& T1, const U& A)
{
...
template< class T > inline T InterpEaseInOut(const T& A, const T& B, float Alpha, float Exp)
{
...
template<> inline ulong_t Min(const ulong_t A, const ulong_t B)
{
	// Negative if B is less than A (i.e. the high bit will be set)
	ulong_t Delta = B - A;
	// Relies on sign bit rotating in
	ulong_t Mask = static_cast<int32_t>(Delta) >> 31;
	ulong_t Result = A + (Delta & Mask);
...
*/
inline INT16 SAddAngleConfined(int32_t Angle, int32_t Delta, int32_t MinThresh, int32_t MaxThresh);
...
struct SGlobalMath
{
	...
	inline float SinTab(int i) const
	{
		return TrigFLOAT[((i >> ANGLE_SHIFT) & (NUM_ANGLES - 1))];
	}
	inline float CosTab(int i) const
	{
		return TrigFLOAT[(((i + 16384) >> ANGLE_SHIFT) & (NUM_ANGLES - 1))];
	}
	inline float SinFloat(float F) const
	{
		return SinTab(appTrunc((F * 65536.f) / (2.f * PI)));
	}
	inline float CosFloat(float F) const
	{
	...
	inline float GetCachedUnit(int32_t Index) const noexcept
	{
		return CachedUnitAngles[Index];
	}
	inline float GetCachedCos(INT16 Angle) const noexcept
	{
		return CachedUnitAngles[98304 + Angle];
	}
	inline float GetCachedSin(INT16 Angle) const noexcept
	{
...
private:
	...
	float  CachedUnitAngles[(UINT16_MAX + 1) * 2];
};
extern SGlobalMath g_SGlobalMath;

inline int32_t ReduceAngle(int32_t Angle);
...
{
	return ((*(ulong_t*)&F1) >= (ulong_t)0x80000000); // Detects sign bit.
}
...
**/
inline BOOL appIsNearlyEqual(float A, float B, float ErrorTolerance = SMALL_NUMBER)
{
...
*	@return					TRUE if A and B are nearly equal
**/
inline BOOL appIsNearlyEqual(double A, double B, double ErrorTolerance = SMALL_NUMBER)
{
...
**/
inline BOOL appIsNearlyZero(float Value, float ErrorTolerance = SMALL_NUMBER)
{
...
*	@return					TRUE if Value is nearly zero
**/
inline BOOL appIsNearlyZero(double Value, double ErrorTolerance = SMALL_NUMBER)
{
...
*/
inline BOOL appIsPowerOfTwo(ulong_t Value)
{
...
struct SAngle
{
	...
	inline void SetValue(const UINT16 val) noexcept
	{
		angle = val;
	}

	inline INT16 Add(const SAngle& other) noexcept
	{
		return (angle += other.angle);
	}
	inline INT16 Sub(const SAngle& other) noexcept
	{
		return (angle -= other.angle);
	}
	inline SAngle operator+(const SAngle& other) const noexcept
	{
		return (angle + other.angle);
	}
	inline SAngle operator-(const SAngle& other) const noexcept
	...
	inline double operator*(const double scale) const noexcept
	{
		return angle * scale;
	}
	inline bool operator<(const SAngle& other) const noexcept
	{
		return (angle < other.angle);
	}
	inline bool operator<=(const SAngle& other) const noexcept
	...
	inline INT16 FromRad(float rad) noexcept
	{
		return (angle = (INT16)(rad * RadToUnit));
	}
	inline INT16 FromDeg(float deg) noexcept
	{
		return (angle = (INT16)(deg * DegToUnit));
	}
	inline float ToRad() const noexcept
	{
		return (float)(angle * UnitToRad);
	}
	inline float ToDeg(float rad) const noexcept
	{
...
struct SIntPoint
{
	int32_t X, Y;
	SIntPoint() {}
	SIntPoint(int32_t InX, int32_t InY)
		: X(InX)
		, Y(InY)
	{}
	...
	SIntPoint& operator*=(int32_t Scale)
	{
		X *= Scale;
		Y *= Scale;
		return *this;
	...
	SIntPoint& operator+=(int32_t Value)
	{
		X += Value;
		Y += Value;
		return *this;
	...
	SIntPoint operator*(int32_t Scale) const
	{
	...
	int32_t Size() const
	{
		return int32_t(appSqrt(float(X * X + Y * Y)));
	}
};
...
struct SIntRect
{
	...
	SIntRect()
	{}
	SIntRect(int32_t X0, int32_t Y0, int32_t X1, int32_t Y1)
		: Min(X0, Y0)
		, Max(X1, Y1)
	{}
	...
	const SIntPoint& operator()(int32_t i) const
	{
		return (&Min)[i];
	}
	SIntPoint& operator()(int32_t i)
	{
		return (&Min)[i];
	}
	static int32_t Num()
	...
	SIntRect Right(int32_t InWidth) const
	{
		return SIntRect(::Max(Min.X, Max.X - InWidth), Min.Y, Max.X, Max.Y);
	}
	SIntRect Bottom(int32_t InHeight) const
	{
	...
	SIntRect& operator*=(int32_t Scale)
	{
		Min *= Scale;
		Max *= Scale;
		return *this;
	...
	SIntRect operator*(int32_t Scale) const
	{
	...
	{
		Min.X = ::Max<int32_t>(Min.X, R.Min.X);
		Min.Y = ::Max<int32_t>(Min.Y, R.Min.Y);
		Max.X = ::Min<int32_t>(Max.X, R.Max.X);
		Max.Y = ::Min<int32_t>(Max.Y, R.Max.Y);

		// Adjust to zero area if the rects don't overlap.
		Max.X = ::Max<int32_t>(Min.X, Max.X);
		Max.Y = ::Max<int32_t>(Min.Y, Max.Y);
	}
	...
	void ExpandFromCenter(int32_t Value) noexcept
	{
		Max += Value;
		Min -= Value;
	}
...
struct SVector2D
{
	...
	inline SVector2D()
	{}
	inline SVector2D(float InX, float InY)
		: X(InX), Y(InY)
	{}

	// Binary math operators.
	inline SVector2D operator+(const SVector2D& V) const
	{
	...
	inline SVector2D operator*(float Scale) const
	{
	...
	inline float operator|(const SVector2D& V) const
	{
		return X * V.X + Y * V.Y;
	}
	inline float operator^(const SVector2D& V) const
	...
	// Unary operators.
	inline SVector2D operator-() const
	{
		return SVector2D(-X, -Y);
	}
	// Assignment operators.
	inline SVector2D operator+=(const SVector2D& V)
	{
		X += V.X; Y += V.Y;
		return *this;
	}
	...
	inline SVector2D operator*=(float Scale)
	{
		X *= Scale; Y *= Scale;
		return *this;
	}
...
struct SVector
{
	...
	inline SVector operator-(float Bias) const
	{
	...
	inline SVector operator*(float Scale) const
	{
	...
	inline SVector operator*=(float Scale)
	{
	...
	*/
	...
	//inline void WriteToStream(MemoryStream* stream) const noexcept {
	//	stream->WriteFloat(X);
	//	stream->WriteFloat(Y);
	//	stream->WriteFloat(Z);
	//}
	...
	inline SVector UnsafeNormal() const
	{
	...
	inline SVector Move2D(const float distance, float radAngle) const noexcept
	{
		return SVector(
	...
	*/
	inline SVector RotateAngleAxis(const int32_t Angle, const SVector& Axis) const
	{
	...
	*/
	...
	inline SVector SafeNormal(float Tolerance = SMALL_NUMBER) const
	{
		const float SquareSum = X * X + Y * Y + Z * Z;
	...
	inline SVector SafeNormal2D(float Tolerance = SMALL_NUMBER) const
	{
		const float SquareSum = X * X + Y * Y;
	...
	*/
	inline SVector ProjectOnTo(const SVector& A) const
	{
		return (A * ((*this | A) / (A | A)));
	}
	...
	*/
	...
	{
		return sqrtf((float)pow(Other.X - X, 2) + (float)pow(Other.Y - Y, 2) + (float)pow(Other.Z - Z, 2));
	}

	float Distance2DTo(const SVector& Other) const noexcept
	{
		return sqrtf((float)((float)pow(Other.X - X, 2) + (float)pow(Other.Y - Y, 2)));
	}
...
struct SVector4
{
	...
	// Binary math operators.
	inline SVector4 operator+(const SVector4& V) const
	{
	...
	inline SVector4 operator*(float Scale) const
	{
	...
	friend inline float Dot3(const SVector4& V1, const SVector4& V2)
	{
		return V1.X * V2.X + V1.Y * V2.Y + V1.Z * V2.Z;
	}
	friend inline float Dot4(const SVector4& V1, const SVector4& V2)
	{
		return V1.X * V2.X + V1.Y * V2.Y + V1.Z * V2.Z + V1.W * V2.W;
	}
	friend inline SVector4 operator*(float Scale, const SVector4& V)
	{
	...
	*/
	...
	inline SVector4 SafeNormal(float Tolerance = SMALL_NUMBER) const
	{
		const float SquareSum = X * X + Y * Y + Z * Z;
		if (SquareSum > Tolerance)
		{
	...
	inline SVector ToVector() const noexcept
	{
		return SVector(
			X,
			Y,
...
public:
	...
	inline SPlane()
	{}
	inline SPlane(const SPlane& P)
		: SVector(P)
		, W(P.W)
	{}
	inline SPlane(const SVector4& V)
		: SVector(V.X, V.Y, V.Z)
		, W(V.W)
	{}
	inline SPlane(float InX, float InY, float InZ, float InW)
		: SVector(InX, InY, InZ)
		, W(InW)
	{}
	inline SPlane(SVector InNormal, float InW)
		: SVector(InNormal), W(InW)
	{}
	inline SPlane(SVector InBase, const SVector& InNormal)
		: SVector(InNormal)
		, W(InBase | InNormal)
	{}
	...
	// Functions.
	inline float PlaneDot(const SVector& P) const
	{
	...
	inline float operator|(const SPlane& V) const
	{
		return X * V.X + Y * V.Y + Z * V.Z + W * V.W;
	}
...
public:
	...
	SSphere()
	{}
	SSphere(int32_t) : Center(0, 0, 0), W(0)
	{}
	SSphere(SVector InV, float InW) : Center(InV), W(InW)
	{}
	SSphere(const SVector* Pts, int32_t Count);
...
public:
	...
	SBox(const SVector* Points, int32_t Count);
	...
	inline SBox& operator+=(const SVector& Other)
	{
		if (IsValid)
		{
			//#if ASM_X86
...
{
	// Variables.
	int32_t Pitch; // Looking up and down (0=Straight Ahead, +Up, -Down).
	int32_t Yaw;   // Rotating around (running in circles), 0=East, +North, -South.
	int32_t Roll;  // Rotation about axis of screen, 0=Straight, +Clockwise, -CCW.

			   // Constructors.
	SRotator() {}
	SRotator(int32_t InPitch, int32_t InYaw, int32_t InRoll)
	...
	SRotator Add(int32_t DeltaPitch, int32_t DeltaYaw, int32_t DeltaRoll)
	{
		Yaw += DeltaYaw;
		Pitch += DeltaPitch;
		Roll += DeltaRoll;
	...
	SRotator AddBounded(int32_t DeltaPitch, int32_t DeltaYaw, int32_t DeltaRoll)
	{
		Yaw += DeltaYaw;
	...
	static int32_t NormalizeAxis(int32_t Angle)
	{
		Angle &= 0xFFFF;
		if (Angle > 32767)
		{
...
public:
	...
	inline SQuat(float InX, float InY, float InZ, float InA)
		: X(InX), Y(InY), Z(InZ), W(InA)
	{}

	//explicit inline SQuat(const SMatrix& M);

	inline SQuat(const SQuat& Q) :
		X(Q.X),
		Y(Q.Y),
		Z(Q.Z),
		W(Q.W)
	...
	//explicit inline SQuat(const SRotator& R);

	/**
	* Copy another SQuat into this one
	*/
	inline SQuat& operator=(const SQuat& Other)
	{
		this->X = Other.X;
		this->Y = Other.Y;
		this->Z = Other.Z;
	...
	*/
	...
	// Binary operators.
	inline SQuat operator+(const SQuat& Q) const
	{
		return SQuat(X + Q.X, Y + Q.Y, Z + Q.Z, W + Q.W);
	}

	inline SQuat operator+=(const SQuat& Q)
	{
		this->X += Q.X;
		this->Y += Q.Y;
		this->Z += Q.Z;
	...
	inline SQuat operator*=(const float Scale)
	{
		X *= Scale;
		Y *= Scale;
		Z *= Scale;
	...
	inline SQuat operator*(const float Scale) const
	{
	...
	*/
	...
	//inline SQuat MakeFromRotator(const SRotator & rotator) const;
	inline SRotator Rotator() const;
};
...
public:
	...
	inline SMatrix();
	inline SMatrix(const SPlane& InX, const SPlane& InY, const SPlane& InZ, const SPlane& InW);
	inline SMatrix(const SVector& InX, const SVector& InY, const SVector& InZ, const SVector& InW);
	...
	inline SMatrix		operator* (const SMatrix& Other) const;
	inline void		operator*=(const SMatrix& Other);
	...
	*/
	inline SMatrix		operator* (float Other) const;
	...
	inline SVector4 TransformFVector4(const SVector4& V) const;

	// Regular transform.

	/** Transform a location - will take into account translation part of the SMatrix. */
	inline SVector4 TransformFVector(const SVector& V) const;

	/** Inverts the matrix and then transforms V - correctly handles scaling in this matrix. */
	inline SVector InverseTransformFVector(const SVector& V) const;
	...
	inline SVector InverseTransformFVectorNoScale(const SVector& V) const;
	...
	*/
	inline SVector4 TransformNormal(const SVector& V) const;

	/**
	*	Transform a direction vector by the inverse of this matrix - will not take into account translation part.
	*	If you want to transform a surface normal (or plane) and correctly account for non-uniform scaling you should use TransformByUsingAdjointT with adjoint of matrix inverse.
	*/
	inline SVector InverseTransformNormal(const SVector& V) const;
	...
	inline SVector InverseTransformNormalNoScale(const SVector& V) const;

	// Transpose.

	inline SMatrix Transpose() const;
	...
	inline SVector GetAxis(int32_t i) const;

	inline void GetAxes(SVector& X, SVector& Y, SVector& Z) const;

	inline void SetAxis(int32_t i, const SVector& Axis);
	...
	inline SVector GetColumn(int32_t i) const;
	...
	inline BOOL GetFrustumNearPlane(SPlane& OutPlane) const;

	inline BOOL GetFrustumFarPlane(SPlane& OutPlane) const;

	inline BOOL GetFrustumLeftPlane(SPlane& OutPlane) const;

	inline BOOL GetFrustumRightPlane(SPlane& OutPlane) const;

	inline BOOL GetFrustumTopPlane(SPlane& OutPlane) const;

	inline BOOL GetFrustumBottomPlane(SPlane& OutPlane) const;
...
inline BOOL MakeFrustumPlane(float A, float B, float C, float D, SPlane& OutPlane)
{
...
inline INT16 SAddAngleConfined(int32_t Angle, int32_t Delta, int32_t MinThresh, int32_t MaxThresh)
{
	if (Delta < 0)
	{
		if (Delta <= -0x10000L || Delta <= -(int32_t)((INT16)(Angle - MinThresh)))
			return MinThresh;
	}
	else if (Delta > 0)
	{
		if (Delta >= 0x10000L || Delta >= (int32_t)((INT16)(MaxThresh - Angle)))
			return MaxThresh;
	}
...
inline SMatrix::SMatrix()
{}

inline SMatrix::SMatrix(const SPlane& InX, const SPlane& InY, const SPlane& InZ, const SPlane& InW)
{
...
inline SMatrix::SMatrix(const SVector& InX, const SVector& InY, const SVector& InZ, const SVector& InW)
{
...
inline void SMatrix::operator*=(const SMatrix& Other)
{
	VectorMatrixMultiply(this, this, &Other);
}

inline SMatrix SMatrix::operator*(const SMatrix& Other) const
{
	SMatrix Result;
...
inline SMatrix	SMatrix::operator*(float Other) const
{
	SMatrix ResultMat;
...
inline void SMatrix::operator*=(float Other)
{
	*this = *this * Other;
}
...
// Homogeneous transform.
inline SVector4 SMatrix::TransformFVector4(const SVector4& P) const
{
	SVector4 Result;
...
// Regular transform.
/** Transform a location - will take into account translation part of the SMatrix. */
inline SVector4 SMatrix::TransformFVector(const SVector& V) const
{
	return TransformFVector4(SVector4(V.X, V.Y, V.Z, 1.0f));
}

/** Inverts the matrix and then transforms V - correctly handles scaling in this matrix. */
inline SVector SMatrix::InverseTransformFVector(const SVector& V) const
{
...
/** Faster version of InverseTransformFVector that assumes no scaling. WARNING: Will NOT work correctly if there is scaling in the matrix. */
inline SVector SMatrix::InverseTransformFVectorNoScale(const SVector& V) const
{
	SVector t, Result;
...
*	If you want to transform a surface normal (or plane) and correctly account for non-uniform scaling you should use TransformByUsingAdjointT.
*/
inline SVector4 SMatrix::TransformNormal(const SVector& V) const
{
	return TransformFVector4(SVector4(V.X, V.Y, V.Z, 0.0f));
}

/** Faster version of InverseTransformNormal that assumes no scaling. WARNING: Will NOT work correctly if there is scaling in the matrix. */
inline SVector SMatrix::InverseTransformNormal(const SVector& V) const
{
	SMatrix InvSelf = this->Inverse();
...
*	If you want to transform a surface normal (or plane) and correctly account for non-uniform scaling you should use TransformByUsingAdjointT with adjoint of matrix inverse.
*/
inline SVector SMatrix::InverseTransformNormalNoScale(const SVector& V) const
{
...
// Transpose.
inline SMatrix SMatrix::Transpose() const
{
	SMatrix	Result;
...
inline SVector SMatrix::GetAxis(int32_t i) const
{
...
inline void SMatrix::SetAxis(int32_t i, const SVector& Axis)
{
	M[i][0] = Axis.X;
	M[i][1] = Axis.Y;
	M[i][2] = Axis.Z;
...
inline SVector SMatrix::GetColumn(int32_t i) const
{
...
// Frustum plane extraction.
inline BOOL SMatrix::GetFrustumNearPlane(SPlane& OutPlane) const
{
...
inline BOOL SMatrix::GetFrustumFarPlane(SPlane& OutPlane) const
{
	return MakeFrustumPlane(
		M[0][3] - M[0][2],
		M[1][3] - M[1][2],
...
inline BOOL SMatrix::GetFrustumLeftPlane(SPlane& OutPlane) const
{
	return MakeFrustumPlane(
		M[0][3] + M[0][0],
		M[1][3] + M[1][0],
...
inline BOOL SMatrix::GetFrustumRightPlane(SPlane& OutPlane) const
{
	return MakeFrustumPlane(
		M[0][3] - M[0][0],
		M[1][3] - M[1][0],
...
inline BOOL SMatrix::GetFrustumTopPlane(SPlane& OutPlane) const
{
	return MakeFrustumPlane(
		M[0][3] - M[0][1],
		M[1][3] - M[1][1],
...
inline BOOL SMatrix::GetFrustumBottomPlane(SPlane& OutPlane) const
{
	return MakeFrustumPlane(
		M[0][3] + M[0][1],
		M[1][3] + M[1][1],
